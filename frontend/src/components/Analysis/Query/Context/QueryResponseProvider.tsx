...
const QueryResponseProvider = ({ children }: QueryResponseProviderProps) => {
  const [accountsQueryData, setAccountsQueryData] =
    useState<AccountsQuery | null>(null);
  const [transactionTypeQuery, setTransactionTypeQuery] =
    useState<TransactionTypeQuery | null>(null);
  ...
  return (
    <QueryResponseContext.Provider value={values}>
      {children}
